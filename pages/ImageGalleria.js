...
const database = SQLite.openDatabase('myImageStore.db');
 ...
 function ImageGalleria({ navigation }) {
    ...
    React.useEffect(() => {
        ...
        database.transaction((tx) => {
            // EXECUTE SQL QUERY TO SELECT DATA FROM 'imageObject' TABLE, ORDERED BY ID DESCENDING
            tx.executeSql(
                'SELECT * FROM imageObject ORDER BY id DESC',
                null,
        ...
        <ScrollView>
            <View style={{ flex: 1, flexDirection: 'row', flexWrap: 'wrap', width: 350 }}>
                ...
                {imageData && imageData.map((image, index) => {
                    return (
                        <TouchableOpacity key={index} onPress={() => navigation.navigate('View Picture', { value: image })} style={{ width: '47%', margin: 5 }}  >
